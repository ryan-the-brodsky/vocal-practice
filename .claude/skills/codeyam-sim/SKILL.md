...
2. **Claude CLI Mode**: If no API key AND `.claude` folder exists → uses Claude Code CLI
3. **Error**: If neither is found → provides helpful error message

### Claude CLI Mode (Default for Claude Code Users)

- **What it is**: CodeYam calls `claude -p` (headless Claude Code) for all AI requests
- **Authentication**: Uses your existing Claude authentication automatically
- **When used**: Automatically when using CodeYam from Claude Code (`.claude` folder detected)
- **How it works**:
  1. Run `codeyam analyze --entity ComponentName 'path/to/file.tsx'`
  2. When analyzer needs AI (key attributes, scenarios, data), it executes: `claude -p "prompt" --output-format json`
  3. Claude CLI returns response
...
- **Setup**: Add your API key to `~/.codeyam/secrets.json`:
  ...
  ```
- **Benefits**: Faster, can run in background without Claude Code

### No Manual Mode Selection Required!

**Mode is detected automatically** - you never need to specify which mode to use:

- ✅ No `--claude-cli` flag needed
- ✅ Works seamlessly in Claude Code (`.claude` folder detected)
...
Once you have the specific entity and file path, run the analysis. CodeYam automatically detects whether to use Claude CLI or direct API mode:
...
The analysis runs autonomously using `claude -p` (headless Claude Code) and opens the browser when complete.
