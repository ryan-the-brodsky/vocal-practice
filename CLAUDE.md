...
`npm test` → 447 tests / 37 suites / 2 projects (1 skipped) in ~85s. `npm run test:coverage` (scoped to unit project) verifies the configured `coverageThreshold` block. `tsc --noEmit` clean. CI at `.github/workflows/test.yml`.
